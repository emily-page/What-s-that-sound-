...
class fourthViewController: UIViewController
{
    ...
    {
        super.viewDidLoad()
        firstOption.setTitle("Perry", for: .normal)
        secondOption.setTitle("Phineas", for: .normal)
        thirdOption.setTitle("Ferb", for: .normal)
    }
    ...
    func playNoise()
    {
        let alertSound = NSURL(fileURLWithPath: Bundle.main.path(forResource: "perry", ofType: "mp3")!)
    ...
    func winAlert()
    {
        ...
        alert.addAction(UIAlertAction(title: "Play Again", style: .default, handler: { (alert) in
            self.firstOption.setTitle("Perry", for: .normal)
            self.secondOption.setTitle("Phineas", for: .normal)
            self.thirdOption.setTitle("Ferb", for: .normal)
        }))
